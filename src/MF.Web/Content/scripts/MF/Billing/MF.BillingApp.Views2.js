...
//            $(this.el).find(".content-header").append($("#payTrainerSearchTemplate").tmpl());
            $(this.el).find(".content-header").prepend('<a href="#" id="acceptSessionsButton"><img src="http://qa.proxy.methodfit.net/csharp/content/images/thumbs_up.jpg" title="Accept Sessions" class="thumbsImage" /></a><a href="#" id="alertAdminButton"><img src="http://qa.proxy.methodfit.net/csharp/content/images/thumbs_down.jpg" title="Email Admin of a problem" class="thumbsImage thumbsLeft" /></a>' );
//            $("#filterArea").empty();
            $(".title-name",this.el).append("<span class='paymentAmount' data-bind='text:paymentAmount'></span>");
            $("[name='EndDate']",this.$el).datepicker();
            this.model.EndDate= ko.observable( new XDate().toString("MM/dd/yyyy") );
...
MF.Views.TrainerPaymentListGridView = MF.Views.View.extend({
    ...
    displayItem:function(id){
        ...
        window.open("http://qa.proxy.methodfit.net/csharp/Billing/PayTrainer/TrainerReceipt/"+id+"?ParentId="+parentId);
        return false;
    },
    viewLoaded:function(){
        this.setupBindings();
