module.exports = function (grunt) {
	...
	grunt.option(
		"srcFolder",
		"../../Projects/" + grunt.option("projectName") + "/"
	);
	...
	grunt.option(
		"deployFolder",
		"/MethodFitness/WebSites/MethodFitness_" + grunt.option("target")
	);
	grunt.option("startTime", new Date());

	grunt.option("QA", {
		connection_String:
			"Server=cannibalcoder.cloudapp.net;Database=MethodFitness_QA;User ID=methodFitness;Password=m3th0d;Connection Timeout=30;",
	...
	grunt.option("PROD", {
		connection_String:
			"Server=localhost;Database=MethodFitness_PROD;User ID=methodFitness;Password=m3th0df1t;Connection Timeout=30;",
	...
	grunt.initConfig({
				...
				files: [
					{
						src: [grunt.option("destFolder") + "/Content/scripts/**/*.js"],
						dest:
							grunt.option("destFolder") +
							"/Content/scripts/concat.min." +
		...
		hbsconfigpoke: {
			compile: {
				...
				files: {
					"<%=destFolder%>/Web.config":
						grunt.option("srcFolder") + "/Web.config.hbs", //,
	...
	grunt.registerTask("logStart", "start time and build params.", function () {
		grunt.log.writeln(
			"grunt build started at: " +
				grunt.option("startTime").toLocaleTimeString()
		);
	...
	grunt.registerTask("logEnd", "End time and build params.", function () {
		grunt.log.writeln(
			"grunt build ended at: " + new Date().toLocaleTimeString()
		);
