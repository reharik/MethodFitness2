...
if (typeof mf.mediaHandler== "undefined") {
            mf.mediaHandler = {};
}

mf.mediaHandler.controller = (function(){
    var setupEvents = function(result,mediaType,availableItems,selectedItems){
        ...
        $(result).find("#"+mediaType+"Input").tokenInput(availableItems?availableItems:{},{prePopulate: selectedItems,
            internalTokenMarkup:function(item){
                var cssClass = tooltipAjaxUrl ? "class='mf_tokenTooltip selectedItem' rel='"+tooltipAjaxUrl+"?EntityId="+item.id +"'":"class='selectedItem'";
                return "<p><a "+cssClass+">"+ item.name +"</a></p>";
            },
            afterTokenSelectedFunction:function(){ $(".mf_tokenTooltip").cluetip({showTitle: false,
