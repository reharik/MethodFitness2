...
function parseMdyyyyHHmmTT(str) {
    if(str.indexOf("t")==-1 || str.indexOf("-")==-1){return null;}
    ...
    if (dateParts.length == 3 && timeParts.length == 2) {
        return new XDate(
            ...
            amPm=="AM"? parseInt(timeParts[0]):parseInt(timeParts[0])+12,// hour
...
function parseMdyyyyHHmmTTSlashes(str) {
    if(str.indexOf("t")==-1 || str.indexOf("/")==-1){return null;}
    var date = str.split("t")[0];
    var time = str.split("t")[1];
    var dateParts = date.split('/');
...
XDate.parsers.push(parseMdyyyyHHmmTT);
XDate.parsers.push(parseMdyyyyHHmmTTSlashes);
